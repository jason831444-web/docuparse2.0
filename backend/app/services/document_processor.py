...
class DocumentProcessor:
    ...
    def _looks_like_non_party_identifier(self, value: Any) -> bool:
        ...
        if re.search(r"(옵션|별도\s*협의|미확정|긴급\s*납품\s*옵션|fast[-_\s]*delivery)", text, flags=re.IGNORECASE):
            return True
        if re.search(r"(담당|당당|검사자|작성자|검수자|사업자\s*번호|사\s*(?:[업엄염연]\s*)?[자지]?\s*[번변]\s*호|등록번호|합계|공급가액|세액|부가세|품목|수량|단가|금액|vendor\s*sku|item\s*code|unit\s*price|line\s*total|\bqty\b|\bspec\b)", text, flags=re.IGNORECASE):
            return True
