...
export function vlCandidateIssueLabel(value?: string | null) {
  ...
  const labels: Record<string, string> = {
    ...
    fax_row_boundary_uncertain: "팩스 행 경계 확인",
...
export function reviewIssueSummary(issue: NormalizedReviewIssue) {
  const labels: Record<string, string> = {
    ...
    internal_item_ambiguous: "내부 품목 후보 참고",
    internal_item_unmatched: "내부 품목 미매칭 참고",
    item_matching_skipped: "내부 품목 매칭 건너뜀",
...
export function reviewIssueDescription(issue: NormalizedReviewIssue) {
  const descriptions: Record<string, string> = {
    ...
    ambiguous_item_match: "사내 품목 master와 정확히 일치하지 않습니다. 필요할 때만 후보를 선택하세요.",
    internal_item_ambiguous: "내부 품목코드 후보가 있으나 필수 입력값은 아닙니다. 필요할 때만 선택하세요.",
    internal_item_unmatched: "내부 품목코드를 확정하지 못했지만 필수 입력값은 아닙니다. 문서에 보이는 품목명/수량을 우선 확인하세요.",
    item_matching_skipped: "품목마스터가 없어 내부 품목코드 매칭을 건너뛰었습니다. 내부코드 없이도 업무데이터 확정은 가능합니다.",
...
export function reviewReasonLabel(value: string) {
  ...
  const labels: Record<string, string> = {
    ...
    internal_item_ambiguous: `${prefix}내부 품목코드 후보 참고`,
    internal_item_unmatched: `${prefix}내부 품목코드 미매칭 참고`,
    ...
    item_matching_skipped: "내부 품목마스터가 없어 품목코드 매칭을 건너뛰었습니다. 내부코드는 필수 입력값이 아닙니다.",
    ...
    visual_crop_or_truncated_column: "오른쪽 금액/세액 컬럼이 잘렸을 가능성이 있어 해당 값은 자동 확정하지 않았습니다.",
    photo_source_review_required: "사진 촬영본으로 감지되어 자동 확정보다 검토가 필요합니다.",
    fax_like_source_review_required: "팩스/저품질 스캔 문서로 감지되어 원본 확인이 필요합니다.",
    no_price_document_amount_blocker: "금액 없는 수량 중심 문서로 판단되어 금액을 임의로 만들지 않았습니다.",
    row_amount_hidden_do_not_infer: "보이지 않는 금액 컬럼은 자동 계산하지 않고 검토 대상으로 남겼습니다.",
    amount_column_not_visible: "금액 컬럼이 보이지 않아 해당 값은 원본 확인이 필요합니다.",
    line_total_not_visible_do_not_infer: "합계금액이 보이지 않아 자동 계산하지 않았습니다.",
    handwritten_vl_candidate: "손글씨 문서에서 읽은 후보값입니다.",
    handwritten_requires_review: "손글씨 문서로 감지되어 자동 확정보다 검토가 필요합니다.",
    handwritten_inspection_requires_review: "손글씨 검사 기록으로 감지되어 수량과 판정을 원본으로 확인해야 합니다.",
    handwritten_amount_missing_or_not_applicable: "손글씨 문서에서 금액 정보가 없거나 업무상 필요하지 않습니다.",
    hold_quantity_requires_review: "보류/불량 수량은 원본 확인이 필요합니다.",
    trailing_number_requires_review: "품목명 뒤 숫자가 수량인지 규격인지 원본 확인이 필요합니다.",
    vl_candidate_handwritten_vl_candidate: "손글씨 문서에서 읽은 후보값입니다.",
    vl_candidate_handwritten_requires_review: "손글씨 문서로 감지되어 자동 확정보다 검토가 필요합니다.",
    vl_candidate_handwritten_inspection_requires_review: "손글씨 검사 기록으로 감지되어 수량과 판정을 원본으로 확인해야 합니다.",
    vl_candidate_handwritten_amount_missing_or_not_applicable: "손글씨 문서에서 금액 정보가 없거나 업무상 필요하지 않습니다.",
    vl_candidate_hold_quantity_requires_review: "보류/불량 수량은 원본 확인이 필요합니다.",
    vl_candidate_trailing_number_requires_review: "품목명 뒤 숫자가 수량인지 규격인지 원본 확인이 필요합니다.",
    vl_candidate_line_total_not_visible_do_not_infer: "합계금액이 보이지 않아 자동 계산하지 않았습니다.",
    validated_candidate_without_known_issues: "자동 검증을 통과한 후보값입니다. 검토 화면에서 최종 확인하세요.",
  };
