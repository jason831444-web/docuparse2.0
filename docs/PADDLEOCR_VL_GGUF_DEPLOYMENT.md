...
Smoke reports also include a `structured_candidate` generated by the same
DocuParse parser rules used for OCR text. This is intentionally serialized under
review candidates only. It must keep `candidate_only=true`,
`parser_integrated=false`, and `confirmed_promotion=false` until a later
validation gate explicitly promotes safe fields.

Exports also include a `promotion_gate` summary for structured VL candidates.
The current gate can mark a candidate as `promotion_eligible`,
`review_required`, or `reject`, but always keeps `auto_promote=false`.
