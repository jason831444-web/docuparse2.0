...
assert.equal(DEFAULT_UPLOAD_CONCURRENCY, 6);
assert.equal(queue.length, 8);
assert.deepEqual(queue.map((item) => item.status), ["selected", "selected", "selected", "selected", "selected", "selected", "selected", "selected"]);
assert.equal(new Set(queue.map((item) => item.id)).size, 8, "duplicate filenames must still get unique queue ids");
assert.deepEqual(nextQueuedUploadIds(queue, DEFAULT_UPLOAD_CONCURRENCY), queue.slice(0, 6).map((item) => item.id));

let running = queue;
for (const id of nextQueuedUploadIds(running, DEFAULT_UPLOAD_CONCURRENCY)) {
  running = markUploadStarted(running, id);
}
assert.equal(runningUploadCount(running), 6);
...
assert.deepEqual(nextQueuedUploadIds(running, DEFAULT_UPLOAD_CONCURRENCY), [queue[6].id]);
...
assert.deepEqual(nextQueuedUploadIds(running, DEFAULT_UPLOAD_CONCURRENCY), [queue[6].id], "accepted documents must be tracked by document id and must not occupy upload slots");
